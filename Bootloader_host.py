...
#BL Commands
COMMAND_BL_GET_VER                                  = 0x0A
COMMAND_BL_GET_HELP                                 = 0x0B
COMMAND_BL_GET_CID                                  = 0x0C
COMMAND_BL_FLASH_ERASE                              = 0x0D
COMMAND_BL_MEM_WRITE                                = 0x0E
...
def calc_file_len():
    size = os.path.getsize("user_app.bin")#binary file to be bootloaded
    return size

def open_the_file():
    global bin_file #make local varaible global
    bin_file = open('user_app.bin','rb')#read-binary
    #read = bin_file.read()
    #global file_contents = bytearray(read)

def read_the_file(): #null operration
    pass
...
def get_crc(buff, length): #CRC algorithm for crc32a
    Crc = 0xFFFFFFFF
    #print(length)
